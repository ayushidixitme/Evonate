...
if (isset($_POST['sbtbtn'])) {
  ...
  $link = mysqli_connect("localhost", "root", "", "AD");
    ...
    $qry = 'SELECT email,UserId from signup where UserId=( select UserId from clothes where id=' . $clothid.')';
  ...
  //Email body
  $mail->Body = '<h1>Hello,<h1> <br/> Hi,<br/> Would you provide ' . $quantity . ' items,  to ' . $_SESSION['First_Name'].' '.$_SESSION['Last_Name']
    . '(' . $_SESSION['Phone_no'] . ').<br/>Please <a href="http://localhost/Mentor/accept_food.php?foodid='.$clothid.'&acc=1&u='.$userid.'&quantity='.$quantity.'"> Accept</a> or <a href="http://localhost/Mentor/accept_cloth.php?clothid='.$clothid.'&acc=0"> reject</a> to preceed. Please reach out to the receiver. <br/><br/><h2>Evonate</h2> ';
...
<?php
  ...
  if (isset($_REQUEST['id'])) {
    $ide = $_REQUEST['id'];
    $link = mysqli_connect("localhost", "root", "", "AD");
    $qry = "select * from clothes join signup on clothes.UserId=signup.UserId where id='$ide'";
...
<form method="post" enctype="multipart/form-data">
    <br>
    <input name="clothid" value="<?php echo $ide; ?>" hidden />
    ...
    <table width="600px" cellspacing="20px">
      ...
      <tr>
        <td colspan="2">
          ...
          <div class="form-group">
            <label>Numbers of Items Needed</label>
            <input type="number" min="1" max="<?php echo $quantity; ?>" class="form-control" name="quantity" placeholder="Number of Items" value="<?php echo  $quantity; ?>" required="" width="350px">
          </div>
        </td>
